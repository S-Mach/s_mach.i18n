...
class OrElseMessagesTest extends FlatSpec with Matchers {
  "Messages.orElse" should "fall back to second Messages if not present in first" in {
    val m1 = Messages(
      Locale.US,
      'key1 -> Format.Literal("test1")
    )
    val m2 = Messages(
      Locale.US,
      'key2 -> Format.Literal("test2")
    )
    val m3 = m1 orElse m2

    m3.get('key1) should equal(Some(Format.Literal("test1")))
    m3.get('key2) should equal(Some(Format.Literal("test2")))
